MODULE usrdef_hgr
   ...
   !!----------------------------------------------------------------------
   !!   usr_def_hgr    : initialize the horizontal mesh for TSUNAMI configuration
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE usr_def_hgr( plamt , plamu , plamv  , plamf  ,   &   ! geographic position (required)
      ...
      !!                TSUNAMI configuration : beta-plance with uniform grid spacing (rn_dx)
      !!
      ...
      !
      IF(lwp) WRITE(numout,*)
      IF(lwp) WRITE(numout,*) 'usr_def_hgr : TSUNAMI configuration bassin'
      IF(lwp) WRITE(numout,*)
