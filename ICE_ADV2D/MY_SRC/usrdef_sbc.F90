MODULE usrdef_sbc
   ...
   !!----------------------------------------------------------------------
   !!   usr_def_sbc    : user defined surface bounday conditions in ICE_ADV2D case
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE usrdef_sbc_oce( kt, Kbb )
      ...
      !!
      !! ** Method  :   all 0 fields, for ICE_ADV2D case
      ...
      IF( kt == nit000 ) THEN
         !
         IF(lwp)   WRITE(numout,*)' usrdef_sbc_oce : ICE_ADV2D case: NO surface forcing'
         ! --- oce variables --- !
         utau(:,:) = 0._wp 
         vtau(:,:) = 0._wp
         taum(:,:) = 0._wp
   ...
   SUBROUTINE usrdef_sbc_ice_tau( kt )
      ...
      INTEGER, INTENT(in) ::   kt   ! ocean time step
      !!---------------------------------------------------------------------
      IF( kt==nit000 .AND. lwp)   WRITE(numout,*)' usrdef_sbc_ice : ICE_ADV2D case: constant stress forcing'
      !
