MODULE usrdef_hgr
   ...
   !!----------------------------------------------------------------------
   !!   usr_def_hgr    : initialize the horizontal mesh for ICE_ADV2D configuration
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE usr_def_hgr( plamt , plamu , plamv  , plamf  ,   &   ! geographic position (required)
      ...
      !!                ICE_ADV2D configuration : uniform grid spacing (rn_dx)
      ...
      !
      IF(lwp) WRITE(numout,*)
      IF(lwp) WRITE(numout,*) 'usr_def_hgr : ICE_ADV2D configuration bassin'
      IF(lwp) WRITE(numout,*)
      IF(lwp) WRITE(numout,*) '          f-plane with regular or irregular grid-spacing (+- 10%)'
